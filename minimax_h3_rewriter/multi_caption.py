            ...
            inputs=[
                *(
                ...
                io.Int.Input(
                    ...
                    optional=True,
                    tooltip=(
                        "0 uses the model's own context, which is what its projector was sized "
                        "against -- one large frame is already twenty-odd media chunks. Set a "
                        "number only to cut the KV cache on a small card; too small a value fails "
                        "the run outright rather than truncating."
                    ),
                ),
                io.Boolean.Input(
                    "bypass",
